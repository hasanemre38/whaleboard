...
export default function Page() {
  ...
  const sorted=[...coins].sort((a,b)=>
    ...
    Math.abs(b.fundRate)-Math.abs(a.fundRate)
  );
  ...
  const s={
    wrap:{background:C.bg,minHeight:'100vh',fontFamily:"'Barlow Condensed',sans-serif",color:C.txt},
    hdr:{background:C.surf,borderBottom:`1px solid ${C.brd}`,padding:'12px 16px',position:'sticky',top:0,zIndex:99,display:'flex',alignItems:'center',justifyContent:'space-between'},
    card:{background:C.surf,border:`1px solid ${C.brd}`,overflow:'hidden',marginBottom:13},
    th:{padding:'8px 10px',fontSize:9,letterSpacing:1.5,textTransform:'uppercase',color:C.mut,fontWeight:400,borderBottom:`1px solid ${C.brd}`,whiteSpace:'nowrap'},
    td:{padding:'10px 10px',fontSize:13,borderBottom:`1px solid rgba(13,31,51,.9)`},
  };

  return (
    <div style={s.wrap}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Barlow+Condensed:wght@400;600;700&display=swap');@keyframes pulse{0%,100%{opacity:1}50%{opacity:.2}}@keyframes spin{to{transform:rotate(360deg)}}.dp{animation:pulse 1.3s infinite}.spin{animation:spin .7s linear infinite}tbody tr:hover td{background:rgba(0,212,255,.04)!important}*{box-sizing:border-box}::-webkit-scrollbar{height:3px}::-webkit-scrollbar-thumb{background:#0d1f33}`}</style>
      ...
      <div style={s.hdr}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          {loading?<div className="spin" style={{width:13,height:13,border:`2px solid ${C.brd}`,borderTopColor:C.acc,borderRadius:'50%'}}/>:<div className="dp" style={{width:8,height:8,background:C.grn,borderRadius:'50%'}}/>}
          <span style={{fontFamily:'monospace',fontSize:10,color:C.mut}}>{lastUpd}</span>
          <span style={{fontFamily:'monospace',fontSize:10,padding:'2px 7px',border:`1px solid ${C.brd}`,color:C.mut}}>{countdown}s</span>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,padding:'13px 13px 0'}}>
        {[
          {lbl:'BTC',val:btc?fmtPrice(btc.price):'—',sub:btc?(btc.chg24>=0?'▲ +':'▼ ')+Math.abs(btc.chg24).toFixed(2)+'%':'—',col:btc?btc.chg24>=0?C.grn:C.red:C.acc},
          {lbl:'ETH',val:eth?fmtPrice(eth.price):'—',sub:eth?(eth.chg24>=0?'▲ +':'▼ ')+Math.abs(eth.chg24).toFixed(2)+'%':'—',col:eth?eth.chg24>=0?C.grn:C.red:C.acc},
          {lbl:'TOPLAM OI',val:coins.length?fmtUSD(coins.reduce((s,c)=>s+c.oiUsd,0)):'—',sub:'Futures OI',col:C.acc},
          ...
          <>
            <div style={{display:'flex',gap:6,marginBottom:12,flexWrap:'wrap',alignItems:'center'}}>
              ...
              {[['oiUsd','OI'],['chg24','24s%'],['vol24','Hacim'],['fundRate','Funding']].map(([k,l])=>(
                <button key={k} onClick={()=>setSort(k)} style={{padding:'4px 9px',background:sort===k?'rgba(0,212,255,.12)':'transparent',border:`1px solid ${sort===k?C.acc:C.brd}`,color:sort===k?C.acc:C.mut,fontSize:10,letterSpacing:1,cursor:'pointer'}}>{l}</button>
              ))}
            </div>
            <div style={s.card}>
                ...
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  <thead>
                    <tr style={{background:'rgba(13,31,51,.7)'}}>
                      {['KOİN','FİYAT','24s%','OI','LONG','SHORT','FON.ORANI'].map((h,i)=>(
                  ...
                  <tbody>
                    {sorted.length===0?(
                      <tr><td colSpan={7} style={{textAlign:'center',padding:28,color:C.mut,fontFamily:'monospace',fontSize:11,letterSpacing:2}}>{loading?'⟳ YÜKLENİYOR...':'Bekleniyor'}</td></tr>
                    ):sorted.map(c=>(
                      <tr key={c.sym}>
                        <td style={{...s.td,fontFamily:'monospace',fontWeight:700,fontSize:14,color:C.white}}>{c.sym}</td>
                        <td style={{...s.td,textAlign:'right',fontFamily:'monospace',color:C.white,fontWeight:700}}>{fmtPrice(c.price)}</td>
                        <td style={{...s.td,textAlign:'right',fontFamily:'monospace',fontWeight:700,color:c.chg24>=0?C.grn:C.red}}>{c.chg24>=0?'▲ +':'▼ '}{Math.abs(c.chg24).toFixed(2)}%</td>
                        <td style={{...s.td,textAlign:'right',fontFamily:'monospace',color:C.txt}}>{fmtUSD(c.oiUsd)}</td>
                        <td style={{...s.td,textAlign:'right',fontFamily:'monospace',color:C.grn,fontWeight:700}}>{c.longPct.toFixed(1)}%</td>
                        <td style={{...s.td,textAlign:'right',fontFamily:'monospace',color:C.red,fontWeight:700}}>{c.shortPct.toFixed(1)}%</td>
                        <td style={{...s.td,textAlign:'right'}}>
                          <span style={{display:'inline-block',padding:'2px 6px',fontFamily:'monospace',fontSize:11,fontWeight:700,background:c.fundRate>=0?'rgba(0,232,122,.1)':'rgba(255,45,85,.1)',color:c.fundRate>=0?C.grn:C.red,border:`1px solid ${c.fundRate>=0?'rgba(0,232,122,.25)':'rgba(255,45,85,.25)'}`}}>{c.fundRate>=0?'+':''}{c.fundRate.toFixed(4)}%</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                ...
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>
                    {!liqs?.liquidations?.length?(
                      <tr><td colSpan={5} style={{textAlign:'center',padding:28,color:C.mut,fontFamily:'monospace',fontSize:11,letterSpacing:2}}>{loading?'⟳ YÜKLENİYOR...':'Bekleniyor'}</td></tr>
                    ):liqs.liquidations.map((l,i)=>(
                      <tr key={i}>
                        <td style={{...s.td,fontFamily:'monospace',color:C.mut,fontSize:12}}>{l.time}</td>
                        <td style={{...s.td,fontFamily:'monospace',fontWeight:700,color:C.white,fontSize:14}}>{l.sym}</td>
                        <td style={{...s.td,textAlign:'right'}}>
                          <span style={{display:'inline-block',padding:'2px 7px',fontFamily:'monospace',fontSize:10,fontWeight:700,background:l.dir==='LONG'?'rgba(255,45,85,.12)':'rgba(0,232,122,.12)',color:l.dir==='LONG'?C.red:C.grn,border:`1px solid ${l.dir==='LONG'?'rgba(255,45,85,.3)':'rgba(0,232,122,.3)'}`}}>{l.dir==='LONG'?'📉':'📈'} {l.side}</span>
                        </td>
                        <td style={{...s.td,textAlign:'right',fontFamily:'monospace',color:C.txt}}>{fmtPrice(l.price)}</td>
                        <td style={{...s.td,textAlign:'right',fontFamily:'monospace',fontWeight:700,color:l.dir==='LONG'?C.red:C.grn,fontSize:14}}>{fmtUSD(l.usd)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
